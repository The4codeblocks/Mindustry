...
public class PrebuildAI extends AIController{
    ...
    private boolean canBuild(CoreBuild core, Block block){
        return world.state.rules.infiniteResources || unit.team.rules().infiniteResources || core.items.has(block.requirements, world.state.rules.buildCostMultiplier);
    }
    ...
    void doMining(){
        ...
        if(mining){
            var targetStack = Structs.find(collectBlock.requirements, i -> !core.items.has(i.item, Mathf.ceil(world.state.rules.buildCostMultiplier * i.amount)));
